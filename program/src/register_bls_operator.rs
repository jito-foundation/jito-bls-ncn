use jito_bls_ncn_core::programs::restaking_core::{Ncn, NcnOperatorState, Config as RestakingConfig};
use jito_bls_ncn_core::utils::{load_account, load_account_mut, JitoAccount};
use jito_bls_ncn_core::{
    accounts::{bls_operator::BlsOperator, rolling_snapshot::RollingSnapshot, config::Config},
...
pub fn process_register_bls_operator(
    ...
    let [config, rolling_snapshot, bls_operator, restaking_config, ncn, operator, ncn_operator_state, admin, system_program] = accounts else {
...

    {
        ...
        RollingSnapshot::check(
            program_id,
            rolling_snapshot,
            true,
        )?;
        let rolling_snapshot_data = rolling_snapshot.try_borrow_data()?;
        let rolling_snapshot_account = unsafe { load_account::<RollingSnapshot>(&rolling_snapshot_data)? };
        ...
        RestakingConfig::check(&jito_bls_ncn_core::programs::restaking_core::id(), restaking_config, false)?;
        let restaking_config_data = restaking_config.try_borrow_data()?;
        let restaking_config_account = unsafe { load_account::<RestakingConfig>(&restaking_config_data)? };
        ...
        NcnOperatorState::check(&jito_bls_ncn_core::programs::restaking_core::id(), ncn_operator_state, false)?;
        let ncn_operator_state_data = ncn_operator_state.try_borrow_data()?;
        let ncn_operator_state_account = unsafe { load_account::<NcnOperatorState>(&ncn_operator_state_data)? };
        ...
        if !ncn_operator_state_account.ncn_opt_in_state.is_active(clock.slot, epoch_length)? {
            msg!("NCN is not opted in");
            return Err(ProgramError::InvalidArgument);
        }
        if !ncn_operator_state_account.operator_opt_in_state.is_active(clock.slot, epoch_length)? {
...

    {
        ...
        let rolling_snapshot_account = unsafe {
            load_account_mut::<RollingSnapshot>(&mut rolling_snapshot_data)?
        };
