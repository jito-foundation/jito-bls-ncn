...
pub async fn add_operators_to_test_ncn(
    ...
    for _ in 0..operator_count {
        ...
        let operator_root = if let Some(fee_bps) = operator_fees_bps {
            ...
            let (operator_pubkey, _, _) =
                jito_bls_ncn_sdk::restaking_sdk::operator_address(&operator_base.pubkey());
            let (config, _, _) = jito_bls_ncn_sdk::restaking_sdk::config_address();
...
pub async fn add_vaults_to_test_ncn(
    ...
    let pass_through = if let Some(token_mint) = token_mint {
        token_mint
    } else {
        Keypair::new()
    };
    ...
    for _ in 0..vault_count {
        ...
        let vault_root = {
            ...
            let (burn_vault, _, _) =
                jito_bls_ncn_sdk::vault_sdk::burn_vault_address(&base.pubkey());
