...
use crate::programs::restaking_core::{Config, Ncn, NcnOperatorState, NcnVaultSlasherTicket, NcnVaultTicket, Operator, OperatorVaultTicket};
...
pub fn ncn_address(base: &Pubkey) -> (Pubkey, u8) {
    let (key, bump, _) = Ncn::find_program_address(&id(), base);
    (key, bump)
}

pub fn operator_address(base: &Pubkey) -> (Pubkey, u8) {
    let (key, bump, _) = Operator::find_program_address(&id(), base);
    (key, bump)
}

pub fn ncn_vault_ticket_address(ncn: &Pubkey, vault: &Pubkey) -> (Pubkey, u8) {
    let (key, bump, _) = NcnVaultTicket::find_program_address(&id(), ncn, vault);
    (key, bump)
}
...
pub fn ncn_vault_slasher_ticket_address(
    ...
    let (key, bump, _) = NcnVaultSlasherTicket::find_program_address(&id(), ncn, vault, slasher);
    (key, bump)
}

pub fn operator_vault_ticket_address(operator: &Pubkey, vault: &Pubkey) -> (Pubkey, u8) {
    let (key, bump, _) = OperatorVaultTicket::find_program_address(&id(), operator, vault);
    (key, bump)
}

pub fn ncn_operator_state_address(ncn: &Pubkey, operator: &Pubkey) -> (Pubkey, u8) {
    let (key, bump, _) = NcnOperatorState::find_program_address(&id(), ncn, operator);
    (key, bump)
}
